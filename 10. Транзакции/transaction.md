...
SELECT current_score INTO _old_score FROM enrollment WHERE id = 1;

-- Увеличиваем балл
UPDATE enrollment
SET current_score = _old_score + 10, updated_at = now()
WHERE id = 1;
...
UPDATE enrollment
SET current_score = _old_score + 15, updated_at = now()
WHERE id = 1;
...
**Шаблон обработки ошибки serialization (в приложении или скрипте):**

**Вариант 1: Простая транзакция с ручной обработкой**
...
-- Если возникает ошибка serialization, выполните:
-- ROLLBACK;
-- И повторите транзакцию заново
```

**Вариант 2: Использование функции для автоматической повторной попытки**

```sql
CREATE OR REPLACE FUNCTION retry_serializable_update(
    enrollment_id_param BIGINT,
    score_increment NUMERIC
) RETURNS VOID AS $$
DECLARE
...
BEGIN
    WHILE NOT success AND retry_count < max_retries LOOP
        BEGIN
            ...
            UPDATE enrollment
            SET current_score = current_score + score_increment, updated_at = now()
        ...
        EXCEPTION
            WHEN serialization_failure THEN
                ...
                IF retry_count >= max_retries THEN
                    RAISE EXCEPTION 'Превышено максимальное количество попыток (%)', max_retries;
                END IF;
...
-- SELECT retry_serializable_update(1, 10);
```

**Результат**: При одновременном выполнении двух транзакций SERIALIZABLE, которые изменяют одни и те же данные, одна из них получит ошибку serialization. Ошибка указывает, что транзакцию нужно повторить. Функция автоматически повторяет операцию до успешного выполнения или до достижения максимального числа попыток.

**Результаты выполнения**: 
- Простая транзакция SERIALIZABLE выполнилась успешно:
  - UPDATE выполнен успешно (1 row affected)
  - COMMIT выполнен успешно
  - Изменения сохранены
- Функция `retry_serializable_update` создана успешно:
  - Функция автоматически обрабатывает ошибки serialization
  - При возникновении ошибки `serialization_failure` делает повторные попытки
  - Максимум 3 попытки с увеличивающейся задержкой
  - При достижении максимума выбрасывает исключение
