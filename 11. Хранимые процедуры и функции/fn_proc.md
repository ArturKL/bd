...
BEGIN
    ...
    VALUES (p_user_id, p_flow_id, now(), 'enrolled');

    -- Записываем его на дисциплины потока
    FOR v_discipline_id IN
        SELECT id FROM discipline WHERE flow_id = p_flow_id
    LOOP
        UPDATE enrollment
        SET discipline_id = v_discipline_id
        WHERE user_id = p_user_id AND flow_id = p_flow_id;
    END LOOP;
...
```
```postgresql
CALL enroll_user_in_flow(15, 4);

SELECT * FROM enrollment WHERE user_id = 15 AND flow_id = 4;
```
...
BEGIN
    ...
    FOR v_enrollment IN
        ...
        FROM enrollment
        WHERE flow_id = p_flow_id AND status = 'enrolled'
           OR status = 'active'
        LOOP
...
```
```postgresql
CALL assign_teacher_to_discipline(16, 3);
SELECT * FROM discipline_teacher WHERE teacher_id = 16;
```
